...
import BuyCollectablesModal from "../BuyCollectablesModal/BuyCollectablesModal";
const CollectablesDetails = ({ isUser }: { isUser?: boolean }) => {
  ...
  return (
    <>
      <BuyCollectablesModal show={show} setShow={setShow} />
      <style jsx global>
        {`
          height: 100%;
          width: 100%;
      ...
      <div className="lg:mx-[80px] my-10 mx-3">
        <div className="flex lg:flex-row flex-col">
          <div className="flex flex-col gap-y-4 lg:gap-y-6">
            ...
            <div className="flex lg:mt-7 flex-col h-[500px] overflow-scroll   lg:w-[500px] w-full gap-y-6 mb-[100px] lg:order-1 order-last">
              {collectables.map((claimable, index: number) => (
                <div
                  //make a function to display the selected collectable on screen
                  key={index}
            ...
            <div className="flex flex-col gap-y-5 mx-12 order-2 justify-center items-center lg:hidden md:hidden xl:hidden">
              <h1 className="text-xl text-[white] z-10 text-center">
                Collectables <br /> Left:
              </h1>
              ...
              <button
                ...
                className="lg:block hidden bg-[#2E3192] border-[3px] border-[#0071BC] lg:m-0 m-2 py-2 px-3 rounded-xl"
              >
                Buy now
              </button>
              <div className="lg:hidden flex absolute z-50 bottom-0 left-[50%] translate-x-[-50%]">
                <button
                  onClick={() => setShow(!show)}
                  className="bg-[#2E3192] border-[3px] border-[#0071BC] lg:m-0 m-2 py-2 px-10 rounded-xl"
                >
                  Buy now
                </button>
              </div>
          ...
          <div className="lg:flex flex-col gap-y-5 mx-12 order-3 hidden">
            ...
            <button
              onClick={() => setShow(!show)}
              className="bg-[#2E3192] border-[3px] border-[#0071BC] lg:m-0 m-2 py-2 px-3 rounded-xl"
            >
              Buy now
            </button>
          </div>
...
export default CollectablesDetails;
